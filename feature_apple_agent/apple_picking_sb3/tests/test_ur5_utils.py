...
import pytest
from apple_picking_sb3.pruning_gym.ur5_utils import UR5
from apple_picking_sb3.pruning_gym import ROBOT_URDF_PATH, MESHES_AND_URDF_PATH
...
@pytest.mark.parametrize("joint_angles, expected_output", [((-np.pi / 2, -2., 2.16, -3.14, -1.57, np.pi), False),
                                                           ((
                                                            -2.0435414506752583, -1.961562910279876, 2.1333764856444137,
                                                            -2.6531903863259485, -0.7777109569760938,
                                                            3.210501267258541), True)])
...
def test_success_collision(ur5, pyb_con, obj_pos_diff, expected_output):
    ...
    tree = pyb_con.loadURDF(urdf_path, [pos[0] + obj_pos_diff[0], pos[1] + obj_pos_diff[1], pos[2] + obj_pos_diff[2]],
                            [0, 0, 0, 1], globalScaling=0.002)
    pyb_con.stepSimulation()
